...
public class StealthPlayerController : Character {
    ...
    void Update() {
        ...
        if (canHover && Input.GetButton("Hover"))
        {
            RaycastHit hit;
            
            if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.down), out hit, Mathf.Infinity, layerMask))
            {
                SpendEnergy(hoverCost * Time.deltaTime);
                if (transform.position.y <= hit.point.y + hoverDistance)
                {
                    transform.position = new Vector3(
                        transform.position.x, 
                        transform.position.y + hoverSpeed * Time.deltaTime, 
                    ...
                    if (transform.position.y >= hit.point.y + hoverDistance)
                    {
                        transform.position = new Vector3(
                            transform.position.x, 
                            hit.point.y + hoverDistance, 
                            transform.position.z);
                    }
                    
                    rb.constraints = RigidbodyConstraints.FreezePositionY | RigidbodyConstraints.FreezeRotation;
                }
                else if(transform.position.y > hit.point.y + 1.5f)
                {
                    transform.position = new Vector3(
                        transform.position.x, 
                        transform.position.y - hoverSpeed * Time.deltaTime, 
        ...
        if (canFire && energy >= fireCost && Input.GetButtonDown("Fire") && (state == States.idle || state == States.moving))
        {
